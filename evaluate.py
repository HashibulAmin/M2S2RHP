...
def evaluate(params, world: MockWorldData, cfg: Config) -> dict:
    ...
    from model.m2s2rec import encode_all_music
    music_reprs = encode_all_music(params, cfg, ctx)
    uids = jnp.arange(cfg.n_users, dtype=jnp.int32)
    rep = user_representation(params, cfg, ctx, uids, situation_probs, content_reprs, music_reprs)
    ...
    scores_flat, _ = score_candidates(
        params, cfg, ctx, all_uids, all_rids, expanded, situation_probs, content_reprs, music_reprs
    )
